"""
Engine constants: piece values, piece-square tables (PeSTO), and search parameters.
...
PeSTO (Piece-Square Tables Only) tables by Ronald Friesen provide positional
bonuses that guide pieces toward good squares (knights toward the center,
kings toward the corners in the middlegame, etc.). Two sets of tables —
middlegame (MG) and endgame (EG) — are blended via tapered evaluation based
on remaining material (see engine/evaluate.py).
"""
...
# ---------------------------------------------------------------------------
# PeSTO (Piece-Square Tables Only) tables by Ronald Friesen, widely used as a
# strong positional baseline. Each table is 64 values indexed by square
# (index 0 = a8, index 63 = h1 — the standard visual board layout with rank 8
# at the top). For a White piece on python-chess square sq (a1=0), use index
# sq ^ 56 to mirror vertically. For a Black piece, use sq directly.
...
# Lookup: piece_type -> (mg_table, eg_table)
# Used by engine/evaluate.py to look up positional bonuses for each piece.
...
# Phase weights for tapered evaluation. Non-pawn material counts toward
# the game phase score. Max phase = 24 (2 queens + 4 rooks + 4 bishops + 4 knights).
